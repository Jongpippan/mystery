...
Follow [the production sequence](00-production-principles.md). Design the entire case, people, 5–7 substantial chapters, player reasoning, and information disclosure before full script production. A small style experiment may happen earlier, but cannot establish the full case or authorize implementation.
...
6. Reconcile chapter-wide and then full-game continuity, knowledge, alternate paths, full required script coverage, and [G0/G1](08-quality-gates.md) before implementation. Codex then continues into adaptation and full implementation without a new user approval request. Convert to runtime data afterward; the Markdown is an authoring contract, not an already implemented importer.
