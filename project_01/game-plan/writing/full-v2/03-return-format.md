Revision amendment: this is the complete r02 return of the supplied r01, governed by00 and08. Existing candidate text must be reviewed/repaired, not counted as newly missing or discarded.

# Full-assignment return contract

YEOWUL-FULL-v2 assigns nineteen final files in07, assembled as YEOWUL-full-draft-r02.zip. These exact output names and full-assignment coverage supersede the generic single-batch examples below. Draft in complete bounded scenes within the same conversation, save progress and continue internally. A response boundary is not a request to return to Codex. If stopped by response limits, report the next unfinished IDs and ask only for `계속` in the same conversation. Preserve a real checkpoint if the environment requires restoration; never claim files/background progress that do not exist.

Each of the nine screenplay files is revision r02, status draft. Notes are English and contain complete scene/branch/hint/source and stable-ID coverage, actual checks, unresolved matters and proposed factual changes. Include exact Korean quotations where needed. The final manifest reconciles all165 C,36 tasks,42 hint bundles/210 levels, alternatives, systems and endings. Do not include a competing rewrite of accepted PR-B01. The writer may revise its earlier candidate units for internal continuity, recording all affected IDs/files; supplied accepted text and case truth remain fixed unless Codex issues an amendment.

The final output must be a real ZIP with all19 complete UTF-8 files. Downloadable attachments are preferred. If file creation is unavailable, explicitly mark text-only delivery and give complete labeled file contents/checkpoints; do not fabricate a download link or mark incomplete work complete. Partial checkpoints are optional safeguards within the writing conversation, not a demand that the user bring each one back to Codex. No fixed number of responses is promised.

## Embedded common format contract
...
Follow the production sequence (`00-production-principles.md`; provenance only). Design the entire case, people, 5–7 substantial chapters, player reasoning, and information disclosure before full script production. A small style experiment may happen earlier, but cannot establish the full case or authorize implementation.
...
6. Reconcile chapter-wide and then full-game continuity, knowledge, alternate paths, full required script coverage, and G0/G1 (`08-quality-gates.md`; provenance only) before implementation. Codex then continues into adaptation and full implementation without a new user approval request. Convert to runtime data afterward; the Markdown is an authoring contract, not an already implemented importer.
...
Official guidance recommends specifying source data, file type, structure, and review criteria when asking ChatGPT for file artifacts; available preview/review features depend on the environment. This contract specifies the desired delivery and an honest fallback, not a guarantee about an uninspected account. See OpenAI: Work with files (`https://learn.chatgpt.com/docs/artifacts-viewer`; provenance only).
