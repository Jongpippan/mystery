...
- Revision: `r01`
- Status: `draft`
- Fixed preceding continuity: accepted `C_PR_01..05` + candidate `C_PR_06..16` + `C_SYS_04`
- Coverage: complete CH01 assignment — `C_CH01_01..08`, `C_CH01_O1..O3`, `C_D01..D05`, `C_Q01`, `C_H_D01..D05`, `C_H_Q01`, `C_H_V01`
...
- Cast: P00, P01, P02
- Time: 10/22 morning
- Entry: The night after C_PR_16. P00/P01 have returned from the town guesthouse for the planned workday. P00 holds E01/E02/E03/E18/E39/E28 and the preserved public-output recording, but no K.
- Exit: P02's casual sketch and E03 cannot both be used as if they describe the same room arrangement. P00 has a concrete reason to inspect L03 rather than treating the discrepancy as a murder clue.
- World effects: none
...
- Entry: C_CH01_01. P00/P01 walk L02→L01→L03 with P02. P04 has not previously been foreground-registered.
- Exit: P04 is registered through actual repair work. E04 and E05 are available. P02 states S_Q01_v1. Objective becomes `창가의 목격이 무엇을 증명하는지 확인하자.` V01 is offered only after K02.
- World effects: acquire E04/E05; optionally acquire E10 from the public album; no K
...
> 바닥 놋쇠 표식 `6`, `9`.
> 최근 바퀴 자국은 두 정지선 사이에 남아 있다.
> 북쪽 창과 남쪽 두 문은 고정.
> 눈금자는 현장 실측용이며, 이 사진만으로 이동 시각은 확정할 수 없다.
...
네. 제가 기억하는 공개 점검 순서 공개 점검 기록하고 같이 볼 때만 그 구간을 말하겠습니다.

*행사 사진과 공개 점검 기록을 함께 가진 경우 C_만실 씨 목격 진술 추궁의 조기 경로가 열린다. 이 경로로 만실 씨 목격 진술 추궁을 먼저 해결해도 창 위치 추론/칸막이 이동 추론의 공간·작업 문제는 별도로 남는다.*
...
### Entry states
- Standard: after C_CH01_02 with E03/E05 held, before C_CH01_03 window comparison.
- Early attempt without E05: free incomplete handling; no 여유 loss.
- Q01 may already be corrected by the E10/E18 route; D01 still concerns room geometry and remains required.

### Task setup

**P00 | 나여백 [S_D01_0001]**

창은 고정이고 칸막이만 6과 9 사이를 움직입니다. 안내도와 실제 정지선을 같이 놓고, 북쪽 창이 각 상태에서 어느 쪽 공간에 들어가는지 먼저 정하겠습니다.
...
고정된 북쪽 창은 x7에 있고, 칸막이는 x6 또는 x9에만 섭니다. 그래서 칸막이가 6에 있으면 창은 칸막이 동쪽, 즉 작업실 쪽에 들어갑니다. 칸막이가 9에 있으면 창은 칸막이 서쪽, 손님들이 쓰는 큰 홀 쪽에 들어갑니다.
...
공개 안내도만으로는 실제 칸막이 정지 위치를 잴 수 없어요. 연회장 연회장에서 두철 씨가 보여 준 바닥의 `6`과 `9`, 그리고 고정된 창·문 위치를 먼저 확인합시다.
...
구분되는 건 창의 위치가 아니라 칸막이가 창의 서쪽에 서는지 동쪽에 서는지예요. x7의 창을 기준으로 x6과 x9를 각각 놓아 보세요.
...
먼저 없는 자료를 채우세요. 행사장 안내도는 봉 선생님의 공개 안내도, 두 정지선은 연회장의 두 정지선 실측입니다. 확보한 뒤 제출할 주장은 `북쪽 창은 x7에 고정이고, x6에서는 동쪽 작업실, x9에서는 서쪽 공개 공간에 포함된다`입니다. 행사장 안내도는 방향/문 이름, 두 정지선은 실측 위치를 맡습니다.

**If all held**

**P00 | 나여백 [S_H_D01_0042]**

제출할 주장은 이렇게 만들 수 있어요. `북쪽 창은 x7에 고정이다. 칸막이가 x6이면 창은 동쪽 작업실에 포함되고, x9이면 창은 서쪽 공개 공간에 포함된다. 행사장 안내도의 방향 정보와 두 정지선의 실측 정지선이 그 구분을 뒷받침한다.`

**If K01 already acquired**

**P00 | 나여백 [S_H_D01_0043]**

창 위치 추론은 이미 끝났습니다. 다시 제출하지 말고 북쪽 창과 칸막이 위치 결론을 현재 전제로 사용하세요. 다음 질문은 어젯밤 실제 칸막이 작업 이력입니다.
...
### Context
- Place: L04 north viewing walk
- Cast: P00, P02, P07
- Entry: K01 after D01. P07 previously authorized a safe sightline inspection. P01 stays in L02 under P03's ordinary reception-area supervision and does not enter the adult inspection route.
- Exit: E06 and E07 are acquired. Q01 is now solvable through E06/E07+K02 once K02 exists, or may already have been solved through E10/E18.
- World effects: acquire E06/E07; no automatic KQ01

### Script

*진새벽은 현관·접수대에서 여백의 요청을 듣고 짧은 질문 카드를 한 장 쓴다. `창밖에서 실제로 무엇까지 보이는가`. 카드를 주머니에 넣으려다 다른 카드 밑에 끼워 넣고는, 결국 찾지 못한 채 말로 다시 확인한다.*
...
모눈이는 휴게실에 남겼습니다. 차무록 씨가 현관 쪽 업무를 보고 있고, 공개된 접수대 안에서만 있게 했어요.

**P07 | 진새벽 [S_CH01_03_0003]**

확인했습니다. 그럼 세 분만 북쪽 보행로로 가죠.

*여백과 만실, 새벽은 연회장 손님 출입문으로 나와 바깥을 돌아 북쪽 관람 통로로 걷는다. 창은 방 바깥에서 허리보다 조금 높은 위치에 있고, 아래쪽은 반투명 처리되어 있다. 내부의 이동식 작업등은 경찰이 허용한 시험 위치에만 놓여 있다.*
...
*첫 시험에서는 빈 의자만 있다. 두 번째에는 파란 재킷을 등받이에 걸고 한쪽 소매가 반투명 창 위로 조금 올라오게 둔다. 세 번째와 네 번째에는 성인 한 명이 앉았다가 일어선다. 앉았을 때는 작업등 몸체가 얼굴 높이 일부를 가린다.*

**P00 | 나여백 [S_CH01_03_0006]**

재킷만 걸었을 때도 위쪽에 소매가 보입니다. 앉은 사람은 얼굴이 작업등에 가려지고요. 서면 어깨와 머리 위치가 달라집니다.
...
창밖에서 보이는 모양을 확인했어도, 어젯밤 벽이 언제 어느 정지선에 있었는지는 칸막이 이동 추론에서 따로 확인해야 합니다.
...
### Entry states
- Standard: after K01 with E04/E05 and P04/P02 both available at L03.
- Q01 may already be corrected through E10/E18. That does not waive D02.
...
지금은 `움직일 수 있다`와 `어젯밤 그렇게 움직였다`가 섞여 있어요. 북쪽 창과 칸막이 위치 결론의 구조, 칸막이 작업표의 작업 시각, 두 정지선의 정지선을 다 놓고 다시 적겠습니다.
...
그럼 두 목격 때 칸막이는 이미 9였던 상태로 놓고 봐야 합니다.
...
먼저 창 위치 추론에서 창과 칸막이 위치 관계를 정해야 합니다. 벽이 6과 9에서 어떤 공간을 만드는지 모르면 작업표의 `정지선 9` 의미도 흐려져요.
...
북쪽 창과 칸막이 위치 결론이 없으면 창 위치 추론부터, 칸막이 작업표/두 정지선이 없으면 연회장의 작업표·정지선부터 채우세요. 다 갖춘 뒤에는 새 바퀴 자국을 정확한 시계로 쓰거나 사건이 있다는 이유만으로 `범인이 혼자 옮겼다`고 붙이지 않습니다.
...
없는 전제가 있으면 먼저 확보하세요. 북쪽 창과 칸막이 위치 결론은 창 위치 추론, 칸막이 작업표는 두철 씨의 작업표, 두 정지선은 연회장 실측입니다. 그 뒤 최종 주장은 `19:40–19:46 탁두철이 작업하고 봉만실이 보조·확인하여 x9로 옮겼고, 칸막이 작업표와 두 정지선이 서로 다른 역할로 이를 뒷받침한다. 두 목격 사이 비밀 이동의 근거는 아니다`입니다.
...
### Context
- Place: L03
- Cast: P00, P04, P02; P01 behind the established safety line
- Entry: K02. P07 approved a neutral partition demonstration in C_CH01_03. P00 explicitly joins V01; no task submission is running.
- Exit: V01 completes x9→x6→x9 and restores the actual current x9 configuration. Test images are labeled as 10/22 experiment. If Q01 is unresolved, both proof routes remain available; if Q01 was corrected early, no second correction is forced.
- World effects: complete V01; no new K

### Script

*연회장으로 돌아오자 노란 안전선이 칸막이에서 충분히 떨어진 위치에 다시 붙어 있다. 모눈은 선 바깥, 휴게실 쪽과 이어지는 손님 출입문 가까이에 서 있다. 여백은 모눈이 선을 넘지 않는 것을 확인한 뒤 두철과 만실 쪽으로 간다.*
...
네. 그 결론을 다시 제출할 필요는 없습니다. 오늘 시험은 왜 방을 잘못 불렀는지와, 어젯밤 벽 상태를 분리해 확인한 겁니다.
...
중간에 멈췄다면 벽을 임의로 원상복구한 것으로 가정하지 않겠습니다. 저장된 시험 상태부터 확인하고 남은 단계만 끝내죠.
...
행사 전 칸막이 이동 결론까지 확인했습니다. 연회장으로 돌아가 두철 씨와 봉만실 선생님에게 오늘 재현시험을 시작하겠다고 말하면 됩니다. 진행 중인 추론 제출이 있다면 끝내거나 보류한 뒤 들어가요.
...
시험 도중 멈췄다면 연회장의 저장된 칸막이 재현 상태로 돌아가세요. 이미 끝낸 이동을 처음부터 반복하지 않고, 남은 단계만 이어갑니다.
...
지금 저장 상태가 x6라면 그건 오늘 시험 중 상태입니다. 어젯밤 상태로 해석하지 말고, 비교를 마친 뒤 x9 복원 단계로 이어가세요.
...
중단 시점의 실제 벽 위치를 먼저 확인하세요. x6까지 갔다면 그 상태 비교를 끝낸 뒤 x9로 복원합니다. 세계 상태를 임의로 리셋하지 않습니다.
...
연회장에서 `재현시험 시작`으로 칸막이 재현에 참가하세요. `시험 전 x9 → 시험 중 x6 → 시험 후 x9`를 확인하고 x9 복원까지 끝냅니다. 이미 확보한 북쪽 창 시야/의자에 걸린 재킷이 있으면 다시 획득하지 말고 그 기록을 열어 보세요. 만실 씨 목격 진술 추궁이 아직이면 북쪽 창 시야/의자에 걸린 재킷+행사 전 칸막이 이동 결론으로 목격 범위를 제출하고, 사진 경로로 이미 만실 씨 목격 진술 추궁을 고쳤다면 그 정정을 반복하지 말고 남은 공간 질문으로 돌아갑니다.

**Already completed and E06 held**

**P00 | 나여백 [S_H_V01_0042]**

칸막이 재현과 북쪽 창 시야까지 이미 갖고 있습니다. 칸막이는 x9로 복원돼 있어야 합니다. 만실 씨 목격 진술 추궁 미완료면 북쪽 창 시야/의자에 걸린 재킷+행사 전 칸막이 이동 결론 경로로 들어가고, 만실 씨 목격 진술 추궁 완료 상태면 다음 미완료 D로 이동하세요.

**Interrupted after x6 image**

**P00 | 나여백 [S_H_V01_0043]**

저장된 `시험 중 x6` 장면에서 재개해 고정 창 위치를 확인한 뒤, 두철 씨와 봉만실 선생님이 x9로 복원하는 단계까지 끝내세요. 그 다음에만 칸막이 재현 완료로 표시하고 북쪽 창 시야/만실 씨 목격 진술 추궁 흐름을 이어갑니다.
...
### Entry states
- **Early route:** after C_CH01_02 if E10+E18 are held. D01/D02/V01 may still be pending.
- **Standard route:** after C_CH01_04 if E06+E07+K02 are held.
- Both routes target the same original S_Q01_v1 and create the sole S_Q01_v2/E08. Success is idempotent; the other route becomes supporting material, not a second correction.
...
봉 선생님, `계속 있었다`는 말의 근거를 두 번 본 장면으로 한정해서 묻겠습니다. 그 사이를 계속 지켜본 건가요, 아니면 떨어진 두 순간을 본 건가요?

**P02 | 봉만실 [S_Q01_0002]**

계속 창 앞에 붙어 있던 건 아니에요. 행사 준비 전에 한 번, 공개 점검이 끝난 뒤에 한 번 봤고, 나는 둘 다 무록 씨가 그대로 있는 거라고 생각했어요.

**P00 | 나여백 [S_Q01_0003]**

좋습니다. 지금은 정확한 시각이나 무엇을 봤는지 제가 대신 채우지 않겠습니다. 두 장면이 `계속 재실`을 증명하는지만 따지겠습니다.
...
행사 전 칸막이 이동 결론대로 두 목격 때 칸막이는 이미 x9였습니다. 북쪽 창 시야 시험에서는 빈 의자에 재킷만 걸어도 창 위로 소매가 보였고, 앉은 사람의 얼굴은 작업등에 가려졌습니다. 의자에 걸린 재킷에는 실제 발견 당시 차무록 씨 재킷이 의자에 걸려 있었습니다. 그러니 창에서 비슷한 모습을 두 번 본 것만으로 그 사람이 그 사이 계속 그 자리에 있었다고 확정할 수 없습니다.
...
초기에는 `계속 있었다`고 기록했고, 만실 씨 목격 진술 추궁 뒤에는 두 번의 제한된 목격으로 정정됐습니다. 둘 사이의 변화 자체가 기록입니다.
...
프롤로그에서 제가 직접 본 공개 점검 순서 공개 점검 기록은 이미 있습니다. 봉 선생님에게 어젯밤 즉석 사진첩을 보여 달라고 하면, 그 구간에 무록 씨가 다른 위치에 있었는지 직접 비교할 수 있어요.

**If standard route missing E06/E07**

**P00 | 나여백 [S_H_Q01_0002]**

사진 경로를 쓰지 않는다면, 북쪽 관람 통로에서 실시한 안전한 창 시야 시험과 진새벽이 보관한 의자·재킷 회수 기록이 필요합니다. 먼저 칸막이 이동 추론으로 어젯밤 칸막이 상태도 확인하세요.

**If K02 missing for standard route**

**P00 | 나여백 [S_H_Q01_0003]**

창 시험만으로는 어젯밤 어느 쪽 공간이었는지 붙일 수 없습니다. 칸막이 이동 추론에서 행사 전 칸막이가 x9로 옮겨졌다는 행사 전 칸막이 이동 결론을 먼저 확인하세요. 또는 행사 사진+공개 점검 기록의 직접 반례 경로를 쓸 수 있습니다.
...
두 목격 사이가 비어 있다는 점에 집중하세요. 직접 다른 위치의 장면을 본 자료가 있거나, 같은 겉모습이 사람 없이도 만들어질 수 있다는 시험이 있으면 `연속 재실`은 별도 증명이 필요해집니다.
...
행사 사진에는 공개 점검 무렵 서비스 접속부 쪽 입구의 차무록 씨가 있고, 공개 점검 기록은 제가 그 공개 점검 준비와 순서를 직접 본 기록입니다. 사진 자체 시각표시가 아니라 두 자료의 실제 맥락을 함께 쓰세요.

**If route B held**

**P00 | 나여백 [S_H_Q01_0022]**

북쪽 창 시야는 오늘 창 시야 시험, 의자에 걸린 재킷은 어젯밤 의자에 걸린 차무록 씨 재킷의 회수 기록입니다. 행사 전 칸막이 이동 결론이 어젯밤 칸막이가 x9였다는 조건을 붙입니다. 시험과 현장 기록을 같은 출처처럼 합치지는 마세요.
...
먼저 한 경로를 완성하세요. 행사 사진을 봉 선생님 사진첩에서 확보해 공개 점검 기록과 잇거나, 북쪽 창 시야/의자에 걸린 재킷을 확인하고 칸막이 이동 추론의 행사 전 칸막이 이동 결론까지 갖춥니다. 그 다음에도 결론은 `연속 재실을 증명하지 못한다`까지이고, 공모 의도는 붙이지 않습니다.
...
방금은 불확실한 목격을 곧바로 공모 의도로 바꿨습니다. `연속 재실을 증명하지 못한다`까지만 제출하고, 왜 잘못 말했는지는 지금 단정하지 마세요.
...
지금 바로 정답 문장을 제출하기보다 한 경로를 먼저 완성하세요. 가장 빠른 현재 경로가 사진이면 봉 선생님 사진첩의 행사 사진을 공개 점검 기록과 연결합니다. 창 경로라면 북쪽 관람 통로의 북쪽 창 시야, 진새벽이 보관한 의자에 걸린 재킷, 칸막이 이동 추론의 행사 전 칸막이 이동 결론을 확보합니다. 확보 뒤 아래 두 완성 주장 중 해당 경로를 그대로 사용할 수 있습니다.
...
만실 씨 목격 진술 추궁은 이미 정정됐습니다. 현재 창가 목격 정정은 `20:08쯤과 20:25쯤 두 번, 얼굴 미확인, 연속 관찰 아님`입니다. 다른 경로를 다시 제출해 두 번째 정정을 만들지 말고 목격 한계 추론 등 다음 미완료 질문으로 이어가세요.
...
### Context
- Place: L04 viewing walk, then L02 return
- Cast: P00, P02, P01
- Entry: Q01 has succeeded and E08/KQ01 exist. If Q01 was solved early, D01/D02/V01/C_CH01_03..04 must still be completed before D03 can resolve the room strand.
- Exit: P02 accepts the limited correction without being converted into a culprit. P01 distinguishes a reserved place from a person's continued presence. D03 is now the active room-alibi question once K01/K02/E07/E08 are held.
...
### Entry states
- Required: K01, K02, E08, E07. E10/E18 may additionally provide the already-known direct counterexample to continuous presence.
- Q01 is already corrected; D03 does not interrogate P02 a second time.
- If any required prerequisite is missing, the attempt remains incomplete without point loss.
...
창가 목격 정정은 두 목격 사이를 지켜보지 않았다고 정정했고, 제가 직접 본 공개 점검 순서 공개 점검 기록과 그때 만들어진 행사 사진에는 차무록 씨가 서비스 접속부 쪽 입구에 있습니다. 북쪽 창과 칸막이 위치 결론/행사 전 칸막이 이동 결론까지 놓으면 창가 자리는 공개 공간 쪽이었으니, 창가의 두 장면을 `잠긴 작업실 안에서 계속 재실`로 읽을 근거도 없습니다. 이건 연속 재실을 부정하는 직접 반례이지, 그가 어디로 더 갔다는 증명은 아닙니다.
...
아까 문장에서 사람의 행동을 너무 멀리 채웠습니다. 초안은 남겨 두고 `목격이 증명하지 못하는 범위`까지만 다시 쓰겠습니다.
...
먼저 창 위치 추론에서 북쪽 창이 x6/x9에서 어느 공간에 속하는지 정하세요. 창가가 잠긴 작업실 쪽인지 공개 공간 쪽인지 모르면 알리바이 범위를 정할 수 없습니다.

**If K02 missing**

**P00 | 나여백 [S_H_D03_0002]**

칸막이 이동 추론에서 어젯밤 행사 전에 칸막이가 x9로 옮겨졌다는 실제 작업 이력을 확인하세요. 오늘 시험 상태를 어젯밤과 섞지 않습니다.

**If E08 missing**

**P00 | 나여백 [S_H_D03_0003]**

만실 씨 목격 진술 추궁에서 봉 선생님의 `계속 있었다` 진술 범위를 먼저 바로잡아야 합니다. 사진 행사 사진+공개 점검 기억 공개 점검 기록 또는 창 시야 북쪽 창 시야/의자에 걸린 재킷+행사 전 칸막이 이동 결론 경로 중 가능한 쪽을 완성하세요.
...
핵심은 `같은 소매가 보였다`와 `같은 사람이 계속 있었다`의 차이입니다. 의자에 걸린 재킷의 재킷은 주인이 없어도 같은 자리에 남을 수 있고, 창가 목격 정정은 두 목격 사이가 비어 있었다고 말합니다.
...
창가 목격 정정은 두 번의 제한된 목격과 연속 관찰 부재를, 의자에 걸린 재킷은 차무록 씨 재킷이 실제 의자에 남아 있던 상태를 보여 줍니다. 북쪽 창과 칸막이 위치 결론/행사 전 칸막이 이동 결론은 그 창가가 당시 공개 공간 쪽이었다는 구조 조건입니다.
...
방금은 모호한 의자 장면에서 범인을 골라 버렸습니다. 목격 한계 추론에는 범행 행동 자료가 없습니다. 재킷 배치의 목적이나 살인자를 말하지 말고 알리바이 범위만 줄이세요.
...
없는 필수 조건부터 채우세요. 북쪽 창과 칸막이 위치 결론은 창/칸막이 공간, 행사 전 칸막이 이동 결론은 어젯밤 x9 작업, 창가 목격 정정은 만실 씨 목격 진술 추궁 정정, 의자에 걸린 재킷은 의자에 걸린 실제 재킷 회수 기록입니다. 확보 뒤 제출할 결론은 `두 목격은 차무록 씨의 연속 재실을 증명하지 않는다. 그러나 누가 재킷을 어떤 목적으로 두었고 누가 범행했는지는 아직 모른다`입니다.

**If all held**

**P00 | 나여백 [S_H_D03_0042]**

이렇게 제출할 수 있습니다. `북쪽 창과 칸막이 위치 결론/행사 전 칸막이 이동 결론으로 두 목격 때 북쪽 창은 x9의 공개 공간 쪽이었다. 창가 목격 정정은 얼굴 없는 두 번의 떨어진 목격뿐이고, 의자에 걸린 재킷의 재킷은 주인이 없어도 같은 소매 모습을 남길 수 있다. 따라서 차무록 씨가 그 사이 계속 방에 있었다는 알리바이는 성립하지 않는다. 범행 여부는 별도다.`

**If K03 already acquired**

**P00 | 나여백 [S_H_D03_0043]**

목격 한계 추론은 끝났습니다. 다음은 실제로 방 밖에서 어디까지 갈 수 있는지 진새벽에게 성인 동선 실측을 요청하세요. 가능 경로를 찾는 것과 실제 사용을 증명하는 것은 다음에도 분리합니다.
...
### Context
- Place: L03 guest door → L08 junction → secured R08 boundary
- Cast: P00, P07, P04
- Entry: K03. P00 explicitly requests V12. P01 remains in L02 with P02 for the entire escorted walk.
- Exit: E09 acquired from the actual walk. The locked eastern workroom strip remains locked; no partition crossing or secret access is introduced. D04 becomes ready.
- World effects: acquire E09; no automatic K04
...
R05는 약 26미터입니다. 보통 걸음으로 여러 번 확인한 범위가 30초에서 45초 정도. 달리기 실험은 하지 않습니다.
...
맞습니다. 동쪽 좁은 작업실 구간은 계속 잠긴 상태로 두고도 R05는 열려 있습니다.
...
아직 실제 걸음을 시작하지 않았다면 연회장 손님 문에서 다시 시작합니다. 지도에 선만 그어 놓고 출입 동선 실측을 획득한 것으로 처리하지 않습니다.

**Interrupted at L08 after R05 measured**

**P07 | 진새벽 [S_CH01_06_0082]**

R05 측정은 끝났습니다. 여기 서비스 접속부에서 이어서 잠긴 서비스 문 바깥쪽을 확인하고, 허용된 수영장 상부 랜딩 통제선까지만 갑니다. 처음부터 다시 걸을 필요 없습니다.
...
### Entry states
- Required: E09 + K01 + K03.
- The question is route feasibility only. It does not ask who used the route.
...
출입 동선 실측에서 실제로 걸은 건 손님 문 쪽 R05입니다. 잠긴 구간을 열지 않았다는 조건을 그대로 지키세요.
...
시험한 건 `갈 수 있다`입니다. 그 시각 그 사람이 걷는 것을 본 목격이나 이동 기록은 지금 제출한 자료에 없습니다.
...
가능 경로는 이 순서입니다. x9 상태의 창가 의자는 서쪽 공개 공간에 있고, 거기서 연회장 손님 출입문으로 나갑니다. 손님 문에서 R05를 따라 서비스 접속부 연결부로 간 뒤, R08로 수영장 상부 랜딩 통제선 방향까지 갈 수 있습니다. 이 과정에서 동쪽 작업실, 서비스 문, 칸막이를 통과할 필요가 없습니다. 출입 동선 실측은 이 경로가 실제로 걸을 수 있음을 보여 주지만 누가 사용했는지는 증명하지 않습니다.
...
> **K04 획득:** 공용 창가 자리에서 손님 문→R05→L08→R08 방향으로 이동 가능한 경로가 있다. 잠긴 동쪽 작업실은 장애가 아니며, 경로의 실제 사용은 미확정.
...
먼저 목격 한계 추론에서 창가 목격이 연속 재실을 증명하지 않는다는 창가 목격의 한계 결론을 정리하세요. 그 뒤 진새벽에게 공용 출구 동선 실측을 요청할 수 있습니다.

**If E09 missing and K03 held**

**P00 | 나여백 [S_H_D04_0002]**

진새벽에게 칸막이 작업 확인 공용 출구 실측을 요청하세요. 모눈은 휴게실에 봉 선생님과 남기고, 성인끼리 연회장 손님 문→서비스 접속부→수영장 상부 랜딩 통제선까지만 실제로 걸어 봅니다.

**If K01 missing**

**P00 | 나여백 [S_H_D04_0003]**

창 위치 추론에서 x9일 때 북쪽 창가가 서쪽 공개 공간에 속한다는 구조부터 확정하세요. 출발 공간을 틀리면 경로가 잠긴 동쪽으로 잘못 들어갑니다.
...
구분할 지점은 `창가에서 어느 문으로 나가느냐`입니다. x9의 창가는 공개 공간이므로 손님 문에 닿고, 그 문 바깥 R05는 잠긴 동쪽 작업실과 별개입니다.
...
출입 동선 실측에는 실제 성인 동선으로 확인한 `연회장 손님 문→R05→서비스 접속부→R08→수영장 상부 랜딩 통제선`이 있고, 동쪽 서비스 문은 잠긴 채 따로 표시돼 있습니다. 시간 수치는 보통 걸음 범위이지 달리기 한계가 아닙니다.
...
방금은 `갈 수 있음`을 `갔음`으로 바꿨습니다. 출입 동선 실측에는 특정 사람의 이동 목격이 없습니다. 경로 성립만 제출하세요.
...
창가 목격의 한계 결론이 없으면 목격 한계 추론을, 출입 동선 실측이 없으면 그 성공 뒤 진새벽의 칸막이 작업 확인 실측을 먼저 끝내세요. 북쪽 창과 칸막이 위치 결론으로 x9의 창가가 공개 공간임을 확인한 뒤 제출할 경로는 `창가 → 연회장 손님 출입문 → R05 → 서비스 접속부 → R08 → 수영장 상부 랜딩 통제선`입니다. 잠긴 동쪽 작업실은 열지 않습니다.

**If all held**

**P00 | 나여백 [S_H_D04_0042]**

완성 주장은 이렇습니다. `북쪽 창과 칸막이 위치 결론/창가 목격의 한계 결론으로 창가의 연속 재실 전제를 뺄 수 있고, 출입 동선 실측에서 x9 공개 공간의 창가에서 손님 문으로 나가 R05를 따라 서비스 접속부, 이어 R08로 수영장 상부 랜딩 방향까지 갈 수 있음을 실제 걸음으로 확인했다. 동쪽 작업실은 잠긴 채다. 따라서 탈출은 물리적으로 가능하지만 누가 사용했는지는 미확정이다.`

**If K04 already acquired**

**P00 | 나여백 [S_H_D04_0043]**

공용 출구 동선 추론은 끝났습니다. 이제 `가능한 경로`를 사람의 죄로 바꾸지 말고, 방에서 확인된 정상 작업·목격 착오·알리바이 한계를 한 번에 정리하는 역할 분리 추론으로 가세요.
...
- Place: L02
- Cast: P00, P01, P03
- Entry: K04. E10 may already be held; if not, the public album remains available from P02 without blocking this scene.
- Exit: P03 gives a fair counterargument: the weakened room alibi and feasible public route do not establish murder or use of R08. The chapter moves toward a local reconstruction rather than premature accusation.
- World effects: no new mandatory E/K
...
그 반박은 현재 기록에 남아 있습니다. 역할 분리 추론에서도 그 빈칸을 억지로 채우지 않겠습니다.
...
- Place: L03
- Cast: P00, P02, P05
- Entry: K04 and C_CH01_07. P01 remains in L02 under P03's ordinary public-area supervision while P00/P02/P05 speak in L03; she does not operate investigation equipment here.
- Exit: D05 success creates K05, fully resolving the room strand. P05 raises the later 20:30 voice as a concrete unresolved reason to inspect the sound booth next; V02 appointment is offered without revealing replay truth.
- World effects: K05 only through C_D05 success; V02 appointment after K05

### Script — before D05

*연회장 연회장에는 칸막이가 시험 전과 같은 x9에 고정돼 있다. 만실은 테이블 위에 `벽`, `목격`, `동선`이라고 쓴 종이 세 장을 펼쳐 둔다. 해금은 케이블 상자를 들고 음향 부스로 돌아가려다가 여백이 부르는 소리에 멈춘다.*
...
좋습니다. 방 문제는 방 구조·목격·이동 역할 분리 결론으로 닫고, 다음 질문은 `늦게 들린 목소리가 그 시각의 사람 존재를 증명하는가`로 넘기겠습니다.
...
### Entry states
- Required: K02, K03, K04.
- Purpose: separate the legitimate wall move, mistaken witness certainty, and weakened alibi beneficiary without converting them into collective guilt.
- Success resolves the CH01 local mystery only; it does not identify the fatal actor.
...
아직 `누가 벽을 옮겼는지`, `누가 무엇을 잘못 봤는지`, `그 결과 어떤 알리바이가 약해졌는지` 중 하나가 비어 있습니다. 빠진 K를 먼저 완성하고 다시 정리하겠습니다.
...
분류표는 그대로 둡니다. `같은 장면에 등장했다`를 `함께 범행했다`로 바꾸거나, `공용 길이 있다`를 `모든 방이 열렸다`로 넓힌 부분만 지우고 다시 제출하겠습니다.
...
방 구조·목격·이동 역할 분리 결론은 첫째 장의 방 문제를 닫는 결론입니다. 벽 작업자, 잘못 본 목격자, 약해진 알리바이의 당사자는 구분됐지만 범행 주체는 아직 정해지지 않았습니다.
...
칸막이 이동 추론에서 어젯밤 칸막이를 실제로 누가, 언제, 어느 정지선으로 옮겼는지 먼저 확인하세요. 정상 작업과 사건 행동을 분리해야 합니다.

**If K03 missing**

**P00 | 나여백 [S_H_D05_0002]**

목격 한계 추론에서 창가 목격이 차무록 씨의 연속 재실을 증명하지 않는다는 범위를 먼저 정하세요. 그 성공이 공용 동선 실측도 엽니다.

**If K04 missing**

**P00 | 나여백 [S_H_D05_0003]**

공용 출구 동선 추론에서 잠긴 동쪽 작업실을 거치지 않고 손님 문→서비스 접속부→수영장 상부 랜딩 방향으로 갈 수 있는 경로를 확인하세요. `가능`까지만 증명하면 됩니다.

**If all held**

**P00 | 나여백 [S_H_D05_0004]**

세 K가 다 있습니다. 이제 사람 셋을 한 팀처럼 묶지 말고 `실제 작업`, `잘못된 목격`, `알리바이 수혜`, `아직 모르는 범행 행동` 네 칸으로 나누세요.
...
없는 K를 먼저 해결해야 합니다. 벽 작업은 칸막이 이동 추론, 알리바이 범위는 목격 한계 추론, 공용 경로는 공용 출구 동선 추론입니다. 미래 장의 음향·사건 현장 자료를 끌어오지 않아도 이 장은 끝낼 수 있습니다.
...
행사 전 칸막이 이동 결론/창가 목격의 한계 결론/공용 출구 동선 가능성 결론을 모두 완성하세요. 그 뒤 제출할 구조는 `정상 벽 작업은 탁두철/봉만실`, `목격 과장은 봉만실`, `그 과장으로 계속 재실처럼 보인 사람은 차무록`, `공용 출구는 가능하지만 실제 사용과 범행 주체는 미확정`입니다.

**If all held**

**P00 | 나여백 [S_H_D05_0042]**

완성 답안은 이렇게 말할 수 있습니다. `탁두철과 봉만실은 행사 전에 정상적으로 칸막이를 x9로 옮겼다. 봉만실은 얼굴 없는 두 목격을 차무록의 연속 재실로 잘못 확장했다. 차무록 재킷은 의자에 남을 수 있고 공용 손님 문을 통한 외부 경로도 가능하다. 따라서 차무록의 방 알리바이는 성립하지 않지만, 누가 경로를 실제로 사용했고 누가 범행했는지는 아직 증명되지 않았다.`

**If K05 already acquired**

**P00 | 나여백 [S_H_D05_0043]**

첫째 장 방 문제는 끝났습니다. 다음은 소해금이 제안한 음향 부스 약속입니다. 늦게 들린 목소리를 사람의 늦은 생존과 바로 같은 것으로 취급하지 말고, 둘째 장에서 실제 음향 자료부터 확인하세요.
...
- Place: L02
- Cast: P00, P01
- Availability: after Q01 correction; optional and skippable. No mandatory clue or K is exclusive here.
- Purpose: relationship/agency callback to P01's lost-property counter and the earlier voice-consent choice.
...
- Place: L03
- Cast: P02, P04, P00
- Availability: after V01/D02; optional. No essential maintenance fact is exclusive here.
- Purpose: ordinary history between owner and contractor; shows postponed repairs without turning them into crime proof.
...
- Place: L01 reception
- Cast: P00, P03
- Availability: after C_CH01_07; optional. No guilt clue or financial proof is exclusive here.
- Purpose: ordinary P03 manager behavior and awkward work priority without ominous framing.
