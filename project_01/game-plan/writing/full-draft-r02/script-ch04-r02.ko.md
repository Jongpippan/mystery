...
- Preceding continuity: revised CH03 r02 with K15, V04 invitation, and the bound wet original remaining in L12 custody
...
- Repeat/resume: if the appointment was postponed, reopen with the unopened envelope; already-read originals are not reacquired
...
*자료실의 넓은 책상 위에 새벽이 투명 자를 내려놓는다. 한술이 가져온 서명 당시 안내문 원본은 보호 슬리브 안에 있다. 살아남은 장부 자체는 L12에 남아 있고, 새벽은 개봉 때 찍은 입회 사진과 페이지 순서 기록만 태블릿에 띄운다.*
...
*한술이 자기 안내문의 오른쪽 아래를 가리킨다. 작은 반달 모양 눌림과 연속 번호가 다음 장으로 이어진다. 새벽이 E26 입회 사진의 같은 위치를 확대한다.*
