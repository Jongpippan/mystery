# Complete r03 return contract

Return YEOWUL-full-draft-r03.zip with the exact19 filenames in00. Each r03 screenplay is the full current file, retaining good supplied working-r02-c01 text and applying targeted revisions. Nine matching English notes plus writing-progress.md must describe actual checked states and per-file retained/new/retired S IDs. Never renumber retained utterances. Keep the accepted opening fixed and out of the return.

Read actual Korean text without metadata and trace affected alternate states; a zero regex result is not narrative acceptance. In notes map F01–F05 to actual scenes/S IDs/selector conditions and explicitly distinguish retained Codex consistency repairs from writer changes. For F01, actual existing Korean may be reused by exact S reference only when its knowledge/cast/meaning fits; no placeholder or unimplemented fallback. Preserve all165 C,71 fixed error keys and210 H identities. A partial unit is draft-incomplete, saved with an exact continuation pointer; only the assembled complete files are returned once.

## Common format — source, not an extra task
