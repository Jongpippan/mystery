'use client';
import {EvidenceList,EvidenceContext} from './EvidenceList';
import {ArchivePanel} from './ArchivePanel';
import {readSaveStore,saveCurrent,createSaveSlot,activateSaveSlot,type SaveSlot} from '@/lib/game/save-store';
...
import { HistoryPanel, PeoplePanel } from './RecordsPanels';
...
export function Game() {
  ...
  useEffect(()=>{const v=savedView.current;if(!v)return;const frame=requestAnimationFrame(()=>{if(bodyRef.current)bodyRef.current.scrollTop=v.scroll;const el=v.focus?document.getElementById(v.focus):null;if(el&&bodyRef.current?.contains(el))el.focus({preventScroll:true});});return ()=>cancelAnimationFrame(frame);},[restoreKey]);
  useEffect(()=>{
    queueMicrotask(()=>{
      try {const store=readSaveStore(localStorage);setSlots(store.slots);setStored(store.slots.find(s=>s.id===store.active)?.state??null);}
  ...
  const startNew=()=>{try{const store=createSaveSlot(localStorage,crypto.randomUUID(),state.started?state:undefined);setSlots(store.slots);setStored(null);setSaveError('');dispatch({type:'start',name:state.started?state.playerName:name});}catch{setSaveError('새 슬롯을 만들 수 없습니다. 기존 기록은 보존됩니다.');}};
  ...
  const portrait=recorded||remote?null:speaker==='P00'?'/art/p00.png':speaker==='P01'?'/art/p01.png':null;
  return <main className="game-root" style={{'--text-scale':state.textScale} as React.CSSProperties}>
    ...
    {!state.started ? <section className="start-screen" style={{backgroundImage:'linear-gradient(90deg,rgba(9,28,29,.94),rgba(9,28,29,.25)),url(/art/l01.png)'}}>
      <div className="start-copy"><p className="eyebrow">여울관</p><h1>남겨 둔 자리</h1><form onSubmit={e=>{e.preventDefault();startNew();}} className="start-form">
      ...
      </form>{slots.some(s=>s.state)&&<div className="save-slots"><h2>보관한 플레이</h2>{slots.filter(s=>s.state).map((s,index)=><Button variant="outline" key={s.id} onClick={()=>loadSlot(s.id)}>{index+1}. {s.state!.playerName} · {archiveComplete(s.state!)?'엔딩':script.scenes[s.state!.sceneId].title}</Button>)}</div>}{saveError&&<p role="alert">{saveError}</p>}</div>
      ...
      <section className="scene-column" aria-label="현재 장면">
        ...
        <div className="dialogue-box" hidden={ended}>
          <div className="dialogue-label">{remote?(originalCall?'보관 창고에서 온 전화 · ':'휴게실 전화 · ')+names[speaker!]:recorded&&node?.kind==='speech'?node.label:speaker ? speaker==='P00'?state.playerName:names[speaker] : node?.kind==='speech'?node.label:scene.title}</div>
          ...
          : node?.kind==='investigation'?<><p className="dialogue-text">{state.investigation?.task?'작성하던 주장이 남아 있습니다. 필요한 자료를 보고 이어갈 수 있습니다.':state.investigation?.stage==='departure'?'음향 부스에서 소해금과 확인할 약속을 잡았습니다.':'확인할 곳과 따져 볼 주장을 선택하세요.'}</p><div className="choice-list">{options.map(o=><Button key={o.id} variant="outline" onClick={()=>o.kind==='visit'?dispatch({type:'visit',stage:o.id}):dispatch({type:'task',id:o.id})}>{o.label}</Button>)}{state.investigation?.task&&<Button onClick={()=>openTool('deduction')}>작성하던 추리로 돌아가기</Button>}</div></>
    ...
    <Dialog open={visibleTool!==null} onOpenChange={open=>{if(!open)closeTool();}}>
      <DialogContent showCloseButton={false} className="tool-dialog" style={{'--text-scale':state.textScale} as React.CSSProperties} onEscapeKeyDown={e=>{e.preventDefault();closeTool();}}>
        ...
        <div className="tool-body" ref={bodyRef} onScroll={e=>{if(state.views?.length)dispatch({type:'viewState',patch:{scroll:e.currentTarget.scrollTop}});}}>
          ...
          {visibleTool==='map'&&<><h3 className="detail-title">여울관 안내도</h3>{state.evidence.includes('E03')?<div className="document-original">{show(script.evidence.E03.text)}</div>:<p className="empty-note">안내도를 받으면 여기서 다시 볼 수 있습니다.</p>}{state.evidence.includes('E09')&&<EvidenceView id="E09" name={state.playerName}/>}<p>지도 열람은 이동하지 않습니다. 현장에서 동행과 출입 허가를 확인한 다음 이동합니다.</p></>}
