...
export function ChallengePanel({state,dispatch}:{state:GameState;dispatch:React.Dispatch<GameAction|InvestigationAction>}) {
  ...
  return <section className="challenge-panel">
    ...
    {t.result?.kind==='incomplete'&&<p role="alert" className="task-feedback">{t.result.message} 여유는 줄지 않았습니다.</p>}
    {state.points===0&&<div className="recovery-panel" role="status"><p>{script.utterances[presentation(state).companion?'S_SYS_01_0011':'S_SYS_01_0021'].text}</p><p>자료·메모·초안은 유지됩니다. 읽고 고치는 것은 무료입니다.</p><Button onClick={()=>dispatch({type:'recover'})}>잠깐 정리하고 이어가기 · 여유 4로 회복</Button></div>}
